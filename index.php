<?php
session_start(); // ثم تبدأ الجلسة
include 'db/db.php'; // استدعاء الاتصال بقاعدة البيانات
...
<?php include 'includes/navbar.php'; ?>
        ...
        <div class="row">
          <div class="col-md-6 ">
            <div class="detail-box">
              <h1>
                Your Trusted <br>
                Home Services Partner. <br>
              </h1>
              <p>
                Explore a wide range of home services for all your needs. From cleaning to repairs, find reliable, fast, and trustworthy providers at your fingertips. Enjoy easy booking and top-quality service, all in one place. To get started, simply sign up or sign in. Let’s begin this exciting journey together.  
                          </p>
            </div>
          </div>
          <div class="col-md-6">
  ...
  <!-- about section -->
  <section id="about_section" class="about_section layout_padding-bottom">
    <div class="container">
        <div class="row">
            <div class="col-lg-5 col-md-6">
                <div class="detail-box">
                    <h2>About us</h2>
                    <p id="short-description-about">
                        There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised.
                    </p>
                    <p id="full-description-about" style="display: none;">
                        There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised. There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised text.
                    </p>
                    <button id="read-more-btn-about" type="button" onclick="toggleText('about')" class="btn btn-primary">Read More</button>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="detail-box">
                    ...
                    <p id="short-description-professional">
                    ...
                    <p id="full-description-professional" style="display: none;">
                    ...
                    <button id="read-more-btn-professional" type="button" onclick="toggleText('professional')" class="btn btn-primary">Read More</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
  function toggleText(section) {
    var fullText = document.getElementById('full-description-' + section);
    var shortText = document.getElementById('short-description-' + section);
    var readMoreBtn = document.getElementById('read-more-btn-' + section);
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
          // تحقق من وجود الأعمدة وتوفير قيم افتراضية إذا كانت غير موجودة
          $image = isset($row['image']) && !empty($row['image']) ? $row['image'] : 'default.png';
          $nom_service = isset($row['nom_service']) ? $row['nom_service'] : 'Service';
          $description = isset($row['description']) ? $row['description'] : 'Description not available.';

          echo '<div class="col-sm-6 col-md-4 mx-auto">
                  <div class="box">
                      <div class="img-box">
                          <img src="images/' . htmlspecialchars($image) . '" alt="Service Image" />
                      </div>
                      <div class="detail-box">
                          <h5>' . htmlspecialchars($nom_service) . '</h5>
                      </div>
                  </div>
                </div>';
      }
...
// تضمين autoload.php الذي ينشئه Composer
require 'vendor/autoload.php'; 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // التحقق من أن الحقول المطلوبة تم تعبئتها
    if ($name && $email && $message) {
        try {
            // إعداد PHPMailer لإرسال البريد الإلكتروني
            $mail = new PHPMailer(true);  // استخدم true لتمكين الاستثناءات
            $mail->isSMTP();  // استخدام بروتوكول SMTP
            $mail->Host = 'smtp.example.com';  // استبدل هذا بـ SMTP server الخاص بك
            $mail->SMTPAuth = true;
            $mail->Username = 'your_email@example.com';  // البريد الإلكتروني
            $mail->Password = 'your_email_password';  // كلمة مرور البريد الإلكتروني
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;  // التشفير
            $mail->Port = 587;  // البورت

            // إعداد البريد الإلكتروني
            $mail->setFrom($email, $name);  // من
            $mail->addAddress('Kahina.arab6789@gmail.com');  // المستلم

            // إعداد الموضوع والمحتوى
            $mail->isHTML(true);
            $mail->Subject = "New Message from $name";
            $mail->Body    = "Name: $name<br>Phone: $phone<br>Email: $email<br><br>Message:<br>$message";
...
<?php if ($success_message): ?>
    <div style="color: green; font-weight: bold; padding-bottom: 15px;">
        <?= htmlspecialchars($success_message) ?>
    </div>
<?php endif; ?>
<?php if ($error_message): ?>
    <div style="color: red; font-weight: bold; padding-bottom: 15px;">
...
<div class="row">
    <div class="col-md-6">
        <form method="POST" action="#contact_section">
            <div>
      ...
      <div class="col-md-6">
        <div class="map_container">
          <div class="map">
            <!-- تضمين خريطة جوجل -->
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3225.9046540191366!2d4.79879437560906!3d36.04703277247129!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x128cbc707a8cfc0b%3A0x6a440e1aeb4d0ce6!2sUniversit%C3%A9%20Bordj%20Bou%20Arreridj!5e0!3m2!1sfr!2sdz!4v1745065539764!5m2!1sfr!2sdz" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
          </div>
        </div>
      </div>
    </div>
